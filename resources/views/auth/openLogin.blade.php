...
<form method="POST" class="login-form" id="login-form" action="{{ route('openLogin') }}">
            @csrf
                ...
                <button type="button" onclick="submitReg();" class="btn bryte-button">{{ __('Login') }}</button>
...
<script>
            // swalWithBootstrapButtons.fire({
            //             html: '<span style="color:#ffffff; font-size: 1.2vw;">Please note that delegates will only be able to login from 09:00</span>',
            //             //html: '<span style="color:#ffffff; font-size: 1.2vw;">This room is closed</span>',
...
function submitReg(){
        ...
        $.ajax({
            ...
            success: function (data) {
                if (data == "1" || data == "This email has already been registered.") {
